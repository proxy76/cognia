...
class UserRepository {
    ...
    fun createOAuthUser(email: String, displayName: String, authProvider: String): UserRow = transaction {
        val id = UUID.randomUUID().toString()
        val now = Clock.System.now().toLocalDateTime(TimeZone.UTC).toString()
        ...
        UsersTable.insert {
            ...
            it[UsersTable.displayName] = displayName
            it[UsersTable.role] = "LEARNER"
            it[UsersTable.authProvider] = authProvider
            it[UsersTable.createdAt] = now
            it[UsersTable.updatedAt] = now
        }
        ...
        UserRow(id, email, null, displayName, "LEARNER", now)
    }
